...
try:
    puerto_serial = serial.Serial('COM8', 9600)  # Cambia COM8 según tu caso
...
def mejorar_colores(frame_bgr):
    ...
    # Aumentar saturación y valor (brillo)
    h, s, v = cv2.split(hsv)
    s = cv2.add(s, 10)  # + saturación
    v = cv2.add(v, 40)  # + brillo
